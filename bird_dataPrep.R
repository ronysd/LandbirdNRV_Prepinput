...
defineModule(sim, list(
  ...
  inputObjects = bindrows(
    ...
    expectsInput(objectName ="roadURL", objectClass = "character", "URL for Canadian road network shapefile.", 
...
Init <- function(sim) {
  ...
  sim$greenupProcessed$greenup_1km <- prepInputs(
    url = sim$greenupURL,
    fun = quote({
  ...
  sim$greenupProcessed$greenup_5km <- terra::focal(sim$greenupProcessed$greenup_1km, w = matrix(1, 5, 5), fun = mean, na.rm = TRUE) |> Cache()
 ...
 browser()
  ...
  sim$topographyProcessed$topography_5km <- terra::focal(sim$topographyProcessed$topography_1km, w = matrix(1, 5, 5), fun = mean, na.rm = TRUE) |> Cache()
  ...
  sim$SCANFI_processed <- processSCANFI(sim$SCANFIurls, sim$studyAreaRas)
...
.inputObjects <- function(sim) {
  ...
  if (!suppliedElsewhere("roadURL", sim)) {
    sim$roadURL <- "https://www12.statcan.gc.ca/census-recensement/2011/geo/RNF-FRR/files-fichiers/grnf000r10a_e.zip"
  }
